...
for a small menu bar app some time last year. Copying livebooks usage of elixir
...
With elixir kit tauri doesn't render an javascript based static/spa website, but
...
The awesome oidcc / oidcc_plug package are provided by the [EEF](https://erlef.org/).
...
phoenix before I knew I wanted to use it in this usecase as well.
...
`redirect_url`. The identity provider then handles authentification of the user.
...
with the identity provider, they do not need to login and are directly forwarded
back to the application they want to login on. On a custom desktop application
the likelyhood of the webview already having an active session is essentially zero.
So we want users to login using the session of their local default browser, not
...
configure the plug, so all the preparation and crypto is still done by oidcc_plugs,
...
has some answeres on that question. It mentions 3 approaches - I'll only go into
...
The path I chose was using elixir kits pubsub again to make tauris rust backend
navigate the webview to the url, where oidcc_plug would pick up the data sent
by the identity provide, validate it and provide it on the auth controller –
...
The RFC mentions this explicitly, but you want that for your an elixir kit app
...
also answers that question: When prividing a url using loopback interfaces
...
makes sense, there's also security considerations to include, … It however also
...
Also deep links do not open on the users browser similartly to e.g. file downloads,
...
privider to be allowed to receive data back after a user authenticated. The
big differenciator when selecting one of those options is if the client can be
...
that's not the case. There's no way for an desktop application to fully keep
secret what it has access to. So when registering an OIDC application to be
used with an desktop app the flow to be used should be the PKCE flow.
...
I've not deeply yet deeply evaluated my current solution for that portion, but
for now I've addressed this one by using rusts keyring to store an encryption
key in the operating systems keyring and deriving an sqlite encryption key and
phoenix secret key from that. If no secret exists it's created.
...
things I should be aware of please let me know. This is my first foreign into
